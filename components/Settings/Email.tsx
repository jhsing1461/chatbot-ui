...
interface Props {
  email: string;
  onEmailChange: (apiKey: string) => void;
}

export const Email: FC<Props> = ({ email, onEmailChange: onEmailChange }) => {
  ...
  const handleUpdateEmail = (newKey: string) => {
    onEmailChange(newKey.trim());
    setIsChanging(false);
  };
